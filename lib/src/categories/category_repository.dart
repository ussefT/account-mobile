...
class CategoryRepository {
  ...
  CategoryRepository({Future<EncryptedStore>? store})
    : _store = store ?? EncryptedStore.create();

  final Future<EncryptedStore> _store;

  Future<List<TxnCategory>> loadAll() async {
    ...
    return decoded
        .whereType<Map>()
        .map((e) => TxnCategory.fromJson(e.cast<String, Object?>()))
        .toList(growable: false);
  }

  Future<void> saveAll(List<TxnCategory> items) async {
    final store = await _store;
